...
public class AuthService {
     ...
     * 发送验证码并返回过期信息。
     * <p>
     * 该方法根据不同的验证场景（注册、登录、重置密码）执行相应的业务逻辑：
     * - 注册场景：要求标识不存在，防止重复注册
     * - 登录/重置密码场景：要求标识存在，确保操作对象有效
     * </p>
     *
     * @param request 请求体，包含：
     *                - scene: 验证场景（注册/登录/重置密码）
     *                - identifierType: 标识类型（手机号/邮箱）
     *                - identifier: 标识值（手机号码或邮箱地址）
     * @return SendCodeResponse 包含以下信息：
     *         - identifier: 目标标识（标准化后）
     *         - scene: 验证场景
     *         - expireSeconds: 验证码过期秒数
     * @throws BusinessException 在以下情况下抛出异常：
     *                           - 标识格式错误（手机号/邮箱格式不正确）
     *                           - 注册场景下标识已存在
     *                           - 登录/重置密码场景下标识不存在
     */
    public SendCodeResponse sendCode(SendCodeRequest request) {
        // 1. 验证标识格式是否符合要求（手机号/邮箱格式校验）
        validateIdentifier(request.identifierType(), request.identifier());
        // 2. 对标识进行标准化处理（如手机号去除空格，邮箱转为小写并去除空格）
     ...
     * @return 认证响应，包含用户信息与令牌对。  用户展示信息和   访问令牌与刷新令牌及其过期时间
    ...
    public AuthResponse register(RegisterRequest request, ClientInfo clientInfo) {
        ...
        // 5. 验证验证码是否正确，验证失败会抛出相应异常
        ensureVerificationSuccess(verificationService.verify(VerificationScene.REGISTER, identifier, request.code()));

        // 6. 构建用户对象，设置基本信息
        User user = User.builder()
                .phone(request.identifierType() == IdentifierType.PHONE ? identifier : null)  // 设置手机号（如果是手机号注册）
                .email(request.identifierType() == IdentifierType.EMAIL ? identifier : null)  // 设置邮箱（如果是邮箱注册）
        ...
        // 7. 如果请求中提供了密码，则进行密码校验和加密存储
        if (StringUtils.hasText(request.password())) {
            validatePassword(request.password());  // 校验密码复杂度
            user.setPasswordHash(passwordEncoder.encode(request.password().trim()));  // 加密存储密码
        }

        // 8. 创建用户到数据库
        userService.createUser(user);

        // 9. 为新注册用户签发JWT令牌对（访问令牌和刷新令牌）
        TokenPair tokenPair = jwtService.issueTokenPair(user);

        // 10. 将刷新令牌加入白名单存储，用于后续的令牌刷新验证
    ...
    public AuthResponse login(LoginRequest request, ClientInfo clientInfo) {
        ...
        // 6. 为登录用户签发JWT令牌对（访问令牌和刷新令牌）
        TokenPair tokenPair = jwtService.issueTokenPair(user);

        // 7. 将刷新令牌加入白名单存储，用于后续的令牌刷新验证
        storeRefreshToken(user.getId(), tokenPair);
