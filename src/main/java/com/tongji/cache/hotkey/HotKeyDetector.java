...
public class HotKeyDetector {
    ...
    /** 每个 key（页面） 的滑窗分段计数数组，长度为 segments    记录页面key与其对应的计数数组*/
    ...
    // 使用computeIfAbsent方法获取或创建一个与key关联的int数组数组
    // 如果key不存在，则使用函数k -> new int[segments]创建一个新的int数组
    // segments表示分段的数量
        int[] arr = counters.computeIfAbsent(key, k -> new int[segments]);
    // 获取当前分段的索引，并将对应分段的计数器加1
    // current.get()用于获取当前线程的分段索引
        arr[current.get()]++;
    }
    ...
    /**
     * rotate方法用于实现滑动窗口统计的核心逻辑。
     * 它会轮转到下一个时间分段，并将当前分段的计数器清零。
     * 定时轮转当前分段，清零新分段以实现滑动窗口统计。
     * 触发频率由配置 `cache.hotkey.segment-seconds` 指定（单位秒）。
     * 这是使用Spring的@Scheduled注解实现定时任务的方法。
     *
     * @Scheduled注解表示这是一个定时任务方法。
     * fixedRateString属性指定了任务的执行间隔，从配置文件中获取"cache.hotkey.segment-seconds"的值，默认为10秒。
     * 注意：配置值后面加了"000"，表示将秒转换为毫秒，因为@Scheduled的fixedRateString单位是毫秒。
     */
    @Scheduled(fixedRateString = "${cache.hotkey.segment-seconds:10}000")
    public void rotate() {
    // 计算下一个分段的索引，使用模运算实现循环
    // current.get()获取当前分段的索引，加1后对总分段数取模，得到下一个分段的索引
        int next = (current.get() + 1) % segments;
    // 更新当前分段索引为下一个分段
        current.set(next);
    // 遍历所有计数器数组，将新分段的计数器清零
    // counters.values()获取所有计数器数组
    // for-each循环遍历每个计数器数组
